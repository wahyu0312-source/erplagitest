...
function setUser(u){
  ...
  // Tampilkan bar (langsung) agar terasa cepat
  ...
  if(allow?.nav){
    ...
    // Only admin dapat Add Member Baru
    if(u.role === 'admin') $("#btnAddMember")?.classList.remove("hidden");

    // Weather & masters boleh idle (non-blocking)
    requestIdleCallback(()=> { ensureWeather(); loadMasters().catch(()=>{}); });
  }

  // Render dash dulu biar terasa ringan, lalu prefetch data secara idle
  show("pageDash");
  refreshAll().catch(()=>{});
  requestIdleCallback(()=>{
    Promise.allSettled([
      cached("listShip",{},30000).then(()=>loadShipsMini()).catch(()=>{}),
      cached("listPlans",{},30000).catch(()=>{}),
      cached("listFinished",{},30000).catch(()=>{}),
      cached("listInventory",{},30000).catch(()=>{})
    ]).catch(()=>{});
  });
}
...
// ================== Login (dipersingkat: tanpa ping awal) ==================
...
async function loadOrders(){
  ORDERS = await cached("listOrders").catch(()=>[]);
  renderOrders();
  loadShipsMini().catch(()=>{});
}
  ...
  (function paint(){
    ...
    for(; i<end; i++){
      ...
      tr.innerHTML = `
        ...
        <td class="center">
          <div class="row">
            <button class="btn icon btn-stqr" title="工程QR"><i class="fa-solid fa-qrcode"></i><span>工程QR</span></button>
            <button class="btn icon btn-scan" data-po="${r.po_id}" title="スキャン"><i class="fa-solid fa-camera"></i><span>スキャン</span></button>
            <button class="btn icon btn-op" data-po="${r.po_id}" title="手入力"><i class="fa-solid fa-keyboard"></i><span>手入力</span></button>
          </div>
        </td>`;
      frag.appendChild(tr);
    }
